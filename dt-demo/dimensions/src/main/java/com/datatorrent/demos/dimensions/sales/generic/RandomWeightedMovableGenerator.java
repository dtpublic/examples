...
/**
 * Provides a mechanism for selecting from a fixed set of objects at random.
 * Each object's probability of being selected is directly proportional to
 * corresponding weight. This weight can be set automatically to default value,
 * or specified manually as objects are added to the collection. A move
 * operation is supported, which allows for random adjustments to respective
 * weights of each object, and changing its probability of being selected. These
 * probability re-distributions of object selection are bound with minWeight and
 * maxWeight values. Random increments during move operation are selected using
 * Gaussian distribution with moveDeviation step size multiplier.
 *
 * @param <E>
 *          Any object to be returned in random weighted fashion
 *
 * @since 2.0.0
 */
class RandomWeightedMovableGenerator<E>
{
  ...
  /**
   * Add new element to this collection with default weight. All elements added
   * with this method will have same probability of being selected.
   * 
   * @param value
   *          value to store
   */
   ...
   *          value to store
   * @param weight
   *          weighted determines probability of selecting this element
   */
   ...
   * 
   * @param localMoveDeviation
   *          movement size multiplier (aka Gaussian standard deviation)
   */
  ...
  {
    if (defaultWeight < minWeight || defaultWeight > maxWeight) {
      return;
    }
